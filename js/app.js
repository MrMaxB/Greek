...
const App = {
  ...
  renderHome() {
    ...
    const ttsWarn = !Speech.supported
      ...
      : (Speech.ready && !Speech.hasGreek())
        ? `<div class="banner">🔈 В системе нет греческого голоса — озвучка может звучать неточно. Для чистого произношения добавьте греческий язык в настройках системы или откройте в Chrome.</div>`
        : "";
    return `
      ${ttsWarn}
      <header class="hero">
  ...
  renderReview() {
    ...
    if (!cur) {
      ...
      // Есть ли ЕЩЁ что повторять/учить прямо сейчас? (иначе «Продолжить» — пустышка)
      const more = SRS.buildQueue(ALL_WORDS).length > 0;
      const nothingYet = s.done === 0 && !more;
      return `
        <div class="result">
          <h2>${nothingYet ? "На сейчас всё ✅" : "Сессия завершена 👏"}</h2>
          ${nothingYet
            ? `<p class="muted">Карточек к повторению нет, а дневной лимит новых слов (${sum.newPerDay}) на сегодня исчерпан. Возвращайся позже — слова придут на повторение по расписанию.</p>`
          ...
          ${more
            ...
            : `<button class="big-btn primary" data-go="track">🗺️ К «Пути»</button>`}
